if (window.location.href.endsWith('create_post/')) {
...
if (sendModer) {
    sendModer.addEventListener('click', async () => {
        ...
        let theme = document.querySelector('.create-post__theme-button')
        const categoryName = theme && theme.textContent ? theme.textContent.trim() : null;
        ...
        if (categoryName && categoryName !== 'Тема') {
...
async function profilePostsFunc() {
    ...
    nextPostsPageUrl = `${BASE_URL}/api/v1/posts/get-user-posts/${currentProfile}`
...
async function getFilterPosts(element) {
    ...
    nextPostsPageUrl = `${BASE_URL}/api/v1/posts/?${params.toString()}`
    isLoadingPosts = false;
...
const searchButton = document.querySelector('.site-header__search-img')
...
async function initScrollForPosts() {
    ...
    nextPostsPageUrl = `${window.BASE_URL}/api/v1/posts/?${params.toString()}`
    nextCommentsPageUrl = `${BASE_URL}/api/v1/comments/?post-pk=${postID}`
