        ...
        if (response.status === 401 || response.status === 403) {
            ...
            <div class="comment-item" id="comment-item-${data.id}">
                ...
                <div class="send_comment__form" id="send_comment__form-${data.id}">
                    <textarea id="comment-input-${data.id}" class="comment-input" placeholder="Комментарий"></textarea>
...
async function initLoadPostComments() {
    ...
    for (let i = 0; i < data.results.length; i++) {
        ...
        <div class="comment-item" id="comment-item-${data.results[i].id}">
            ...
            <div class="send_comment__form" id="send_comment__form-${data.results[i].id}">
                <textarea id="comment-input-${data.results[i].id}" class="comment-input" placeholder="Комментарий"></textarea>
...
async function profileCommentsFunc() {
    ...
    nextCommentsPageUrl = `${BASE_URL}/api/v1/comments/get-user-comments/${user}`
    isLoadingComments = false;
    isLoadingPosts = true;
...
async function setCommentLike(div) {
    ...
    const response = await fetch(`${BASE_URL}/api/v1/comments/${id}/set-like/`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRFToken': window.csrfToken,
