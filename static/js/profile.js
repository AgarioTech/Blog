...
async function profileBookmarksFunc() {
    ...
    nextPostsPageUrl = `${BASE_URL}/api/v1/posts/get-my-bookmarks-posts/${user}`
    nextCommentsPageUrl = `${BASE_URL}/api/v1/comments/get-my-bookmarks-comments/${user}`
...
async function saveProfileData() {
    ...
    const image = document.querySelector('.settings__photo').files[0]
    ...
    if (!image) {
        const image = document.querySelector('.profile-image').src
...
async function profileFollowersFunc() {
    ...
    const response = await fetch(`${BASE_URL}/api/v1/users/${userId}/get-followers`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
        },
